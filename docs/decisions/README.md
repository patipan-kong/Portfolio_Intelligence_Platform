...
| [ADR-007](ADR-007_WEALTH_GOAL_AUTHORITY_AND_LEGACY_PORTFOLIO_GOAL_PROFILE_BOUNDARY.md) | Wealth Goal is the sole canonical life-goal concept; the Portfolio Goal Profile remains a semantically frozen legacy compatibility contract, distinct from any future Portfolio Investment Mandate. |
| [ADR-008](ADR-008_WEALTH_GOAL_DECISION_CONTEXT_ADMISSION.md) | Explicitly selected Wealth Goal context may be admitted to a `RecommendationSnapshot` as a context-only historical record, constructed strictly after the recommendation exists so it cannot influence it. |
| [ADR-009](ADR-009_GOAL_AWARE_RECOMMENDATION_CONSTRAINTS.md) | One explicitly activated Wealth Goal may deterministically tighten the recommendation single-position bound without changing Phase 7.4 context-only selection. |
| [ADR-010](ADR-010_PORTFOLIO_INVESTMENT_MANDATE_FOUNDATION.md) | A user-authored, factual, many-to-many Portfolio↔Wealth Goal relationship records what a Portfolio is managed for, with no behavioral, optimizer, or advisory-cardinality authority. |
| [ADR-012](ADR-012_INVESTMENT_FUNDING_TRANSFER_REPRESENTATION.md) | A cash-side Investment Funding Transfer is a distinct `CashAccountTransaction` type carrying a metadata-only Portfolio counterparty — a Cash Account fact only, with no authority to write, match, or reconcile the Portfolio ledger. |
| [ADR-013](ADR-013_NET_WORTH_CHANGE_ATTRIBUTION_LEVEL1_BOUNDARY.md) | Net Worth Change Attribution explains historical Net Worth movement only at the three-component balance-sheet level (investment assets, external cash, liability impact), with a mandatory reconciliation guard and an AVAILABLE/UNAVAILABLE boundary; Level-2 economic-cause attribution (market return, contribution, debt repayment) is explicitly deferred. |
